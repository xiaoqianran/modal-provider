...
def test_discovers_stable_runtime_roles_for_generated_world_files(tmp_path: Path):
    ...
    semantics = tmp_path / "camera_trajectory/target_camera.json"
    visual = tmp_path / "checkpoints/final.spz"
    other = tmp_path / "navmesh/metadata.json"
    for path in (mesh, semantics, visual, other):
    ...
    assert by_path["camera_trajectory/target_camera.json"].role == "world-semantics"
