...
TAG = "hyworld2-flash-attn-py311-cu128-torch271-sm90-v1"
...
app = modal.App("modal-build-hyworld2-flash-attn-sm90")
...
def build() -> dict:
    ...
    if torch.cuda.get_device_capability() != (9, 0):
        raise RuntimeError("FlashAttention Hopper build must run on sm_90")
    ...
    if out.shape != q.shape or not torch.isfinite(out).all():
        raise RuntimeError("flash-attn sm_90 CUDA smoke failed")
    ...
    manifest = {
        ...
        "smoke": ["gpu-sm90", "flash_attn_func-bf16"],
    }
