...
async function request(path,options={}){
  const headers={...(options.headers||{})};if(state.token)headers["X-Modal-3D-Session"]=state.token;
  let body=options.body;if(body&&!(body instanceof FormData)){headers["Content-Type"]="application/json";body=JSON.stringify(body)}
  let res;try{res=await fetch(path,{...options,headers,body})}catch{setAgent("bad","Client 不可达");throw new Error("无法连接本地 Client")}
  const type=res.headers.get("content-type")||"";const data=type.includes("application/json")?await res.json():type.includes("model/gltf-binary")||type.includes("application/octet-stream")?await res.blob():await res.text();
