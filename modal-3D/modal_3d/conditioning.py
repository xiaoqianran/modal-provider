...
class BackgroundMaskRequired(ValueError):
    """Raised when an opaque source needs Provider background removal."""
...
def condition_image(data: bytes, predicted_mask: bytes | None = None) -> dict[str, object]:
    ...
    a Provider-predicted mask, which is refined using the same BiRefNet tail
    validated by experiment 041 before canonical letterboxing.
    """
