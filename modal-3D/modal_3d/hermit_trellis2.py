...
gpu_image = (
    ...
    .run_commands(
        ...
        "git clone --recursive https://github.com/JeffreyXiang/CuMesh.git /tmp/CuMesh && python -m pip install /tmp/CuMesh --no-build-isolation",
        "git clone --recursive https://github.com/JeffreyXiang/FlexGEMM.git /tmp/FlexGEMM && python -m pip install /tmp/FlexGEMM --no-build-isolation",
        ...
        "cd /opt/hermit && MAX_JOBS=4 python -m pip install ./o-voxel --no-build-isolation",
    )
    .env(
        {
            "PYTHONPATH": SRC_DIR,
